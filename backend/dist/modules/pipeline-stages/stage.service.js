...
export class StageService {
    ...
    static async archiveStage(tenantId, id, transferToStageId) {
        ...
        if (stage.isSystem)
            throw { status: 403, message: 'System stages cannot be archived' };
    ...
    static async migrateBulk(tenantId, sourceId, targetId, userId, reason) {
        ...
        if (!sourceStage || !targetStage)
            throw { status: 404, message: 'Stage not found' };
        if (sourceStage.type !== targetStage.type)
            throw { status: 400, message: 'Stages must be of same type' };
        return await prisma.$transaction(async (tx) => {
            let migratedCount = 0;
    ...
    static async reorderStages(tenantId, items) {
        return await prisma.$transaction(async (tx) => {
            const updates = items.map((item) => tx.pipelineStage.update({
