...
import { UserRole, LeadSource, LeadPriority, DealStatus, TaskStatus, TaskPriority, TaskType, CommunicationType, CommunicationDirection, CommunicationSourceType, ProductType, ProductStatus, ProposalStatus } from '@prisma/client';
...
export async function seedDemoData() {
    ...
    const seeded = await RBACService.seedDefaults(tenant.id, prisma);
    ...
    for (const p of proposalSeedData) {
        ...
        const proposal = await prisma.proposal.create({
            data: {
                ...
                viewedAt: ([ProposalStatus.sent, ProposalStatus.accepted, ProposalStatus.rejected].includes(p.status) ? subDays(new Date(), 2) : null),
                respondedAt: ([ProposalStatus.accepted, ProposalStatus.rejected].includes(p.status) ? subDays(new Date(), 1) : null),
